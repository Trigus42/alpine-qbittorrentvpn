...
DEFAULT_FIREWALL_CHECK_INTERVAL=5
...
if [[ -z "${FIREWALL_CHECK_INTERVAL}" ]]; then
	echo "$(date +'%Y-%m-%d %H:%M:%S') [INFO] FIREWALL_CHECK_INTERVAL is not set. Using default interval of ${DEFAULT_FIREWALL_CHECK_INTERVAL}s"
